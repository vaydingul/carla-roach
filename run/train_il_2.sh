...
wb_project="il_leaderboard_roach" wb_group="Script Test" 'wb_name="Control-only"' \
...
agent.cilrs.training.kwargs.trajectory_weight=0.0 \
...
agent.cilrs.policy.kwargs.use_multi_step_waypoint=false \
agent.cilrs.policy.kwargs.use_trajectory_guided_control=false \
...
agent.cilrs.policy.kwargs.number_of_steps_waypoint=0 \
train_epochs=500 \
cache_dir="$1"
}
