...
# * To benchmark il agents.
 ...
 benchmark () {
   ...
   wb_project=iccv21-roach-benchmark \
   agent=$agent actors.hero.agent=$agent \
   agent.cilrs.wb_run_path=vaydingul/il_leaderboard_roach/2zswgpjq \
   'wb_group="L_K + L_V + L_F(c) Multi-Step LAV"' \
   'wb_notes="Benchmark L_K + L_V + L_F(c) Multi-Step Control Prediction on LAV."' \
