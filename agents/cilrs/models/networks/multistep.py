...
class MultiStepControl(nn.Module):
	...
	def forward(self, j):
		...
		waypoint = self.policy_head_waypoint(j)[0]#torch.zeros((j.shape[0], 2), dtype = j.dtype)
		...
		for _ in range(self.number_of_steps):
			...
			x_in = torch.cat([waypoint, h], dim = 1)
		...
		pred_waypoint, pred_j = torch.stack(waypoint_vector, dim = 1), torch.stack(j_vector, dim = 1)

		return pred_waypoint, pred_j
